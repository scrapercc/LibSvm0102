...
class Info(object):
    def __init__(self, data, param):
        if data != None:
            ...
            # print('approval_score:',self.approval_score)
            # print('doubt_score:',self.doubt_score)

            if int(data.get('friends_count')) > 0 and int(data.get('followers_count')) / int(
                    data.get('friends_count')) >= param and int(data.get('followers_count')) >= 1000:
                self.type = 'o'
            else:
                self.type = 'n'
        else:
...
def approval_doubt_score(text, pos_words, neg_words):
    ...
    #     return round(posCount / (posCount+negCount), 2), round(negCount / (posCount+negCount), 2)
    # else:
    #     return 0,0

def sim(node1, node2,sigma):
    ...
    ind = -(np.linalg.norm(vector_a-vector_b) **2)/(2*sigma**2)
    res = round(float(np.e ** ind),2)
    return res
...
def rwg_kernel(matrix, param_a):
    ...
    e[random.sample(range(0, matrix.shape[0]), 1)] = 1
...
def kernel_preProcess(Train_tree,Test_tree=None,isTraining=True, param_a=0.7, sigma=1.5, myK=0):
    """
    
    :param Train_tree: 
    :param Test_tree: 
    :param isTraining: 
    :param param_a: 原文的lanmbda
    :param sigma: 计算直积矩阵时要计算两边的高斯核
    :param myK: 
    :return: 
    """"""
    :param Train_tree: 
    ...
    if isTraining == True:
        ...
        for i in range(len(Train_tree)):
            ...
            x_dict = {}
            x_dict[0] = i + 1
            i_nodes = Train_tree[i].size()
            for j in range(i, len(Train_tree)):
                j_nodes = Train_tree[j].size()
                print(i, j, i_nodes, j_nodes)
                # print(i, j, "计算直积矩阵")
                product_matrix = cal_product_graph(Train_tree[i], Train_tree[j],sigma=sigma)
                ...
                kernel_matrix[i,j] = round(rwg_kernel_val/(i_nodes*j_nodes),2)
                if i!=j:
                    kernel_matrix[j,i] = round(rwg_kernel_val/(i_nodes*j_nodes),2)

            # write_xdict_toFile(x_dict, './Kernels/xdict_Train{}'.format(myK))
            # print(i, j, "成功写入文件")
            time_end = int(time.time())
            print('time_end:', time_end)
            print('耗时：', (time_end - time_begin) / 60, 'min')
            # del (product_matrix)
            # gc.collect()
            # print()
            # print()
        ...
        for i in range(len(Test_tree)):
            ...
            for j in range(len(Train_tree)):
                # print(i,j,"计算直积矩阵")
                product_matrix = cal_product_graph(Test_tree[i], Train_tree[j],sigma=sigma)
                # print(i, j, "直积矩阵计算完毕")
                # print(i, j, "计算随机游走图核")
                rwg_kernel_val = rwg_kernel(product_matrix, param_a=param_a)
                # print(i, j, "计算随机游走图核完毕")
                ...
                # print(i, j, "计算径向基核完毕")
                x_dict[j + 1] = round(rwg_kernel_val/(Test_tree[i].size()*Train_tree[j].size()),2)
...
def cal_product_graph(tree1, tree2,sigma):
        ...
        for j in range(len(new_nodes)):
            if i != j:
                ...
                if tree1.get_node(node11) == parent_1 and tree2.get_node(node12) == parent_2:
                    ma[i, j] = sim(tree1.get_node(node21), tree2.get_node(node22),sigma=sigma)
...
if __name__ == "__main__":
    ...
    #使用100条数据作为验证集。其中52条虚假新闻，48条真实新闻
    random.seed()
    random_index = random.sample(range(0, 4664), 10)
    ...
    # params = [20,30,35,40,45,50,55,60,65,70]
    params = [35]
    accuracys = []
    for param in params:
        trees = []
        ...
        for i in range(data_array.shape[0]):
            ...
            # 构建新闻事件传播树
            tree = Tree()
            tree.create_node(tag=json_data[0].get("mid"), identifier=json_data[0].get("mid"), data=Info(json_data[0],param=param))
            uu = 1000 if len(json_data) >= 1000 else len(json_data)
            # print('uu:',uu)
            for j in range(1, uu):
                try:
                    tree.create_node(tag=json_data[j].get("mid"), identifier=json_data[j].get("mid"),
                                     parent=json_data[j].get("parent"), data=Info(json_data[j],param=param))
                except:
                    pass
            # 单颗传播树构建完成
            # 对传播树进行简化
    ...
    # plt.savefig('./img/img_1.jpg')
        ...
        kf = ShuffleSplit(n_splits=1, random_state=0, test_size=0.2)
        myK = 0
        ...
        for train, test in kf.split(wb_data):
            ...
            print('train_count_1:',np.sum(wb_data[train][:,1]==1))
            print('train_count_0:', np.sum(wb_data[train][:, 1] == 0))
            ...
            train_data = kernel_preProcess(Train_tree=train_trees,isTraining=True)
            test_data = kernel_preProcess(Train_tree=train_trees,Test_tree=test_trees,isTraining=False)
            # print('train_data:',train_data[:5])
            # print('test_data:',test_data)
            print('train_data:')
            for x in range(len(train_data)):
                print(train_labels[x],train_data[x])
            print('test_data:')
            for x in range(len(test_data)):
                print(test_labels[x],test_data[x])
            print()
            # print('train_data:',train_data)
            # print('test_data:',test_data)

            # kernel_preProcess(Train_tree=train_trees,isTraining=True, myK=myK)
            # kernel_preProcess(Train_tree=train_trees,Test_tree=test_trees,isTraining=False, myK=myK)
            # 根据自定义的核函数训练模型
    ...
    plt.plot(params, accuracys,'ro-')
    plt.xlabel('alpha')
    plt.ylabel('accuracy')
    plt.show()
    # plt.savefig('./img/img_2.jpg')